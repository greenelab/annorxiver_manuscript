...
To understand how they relate to the traditional publishing ecosystem, we examine the lingustic similarities and differences between preprints and peer reviewed text and how lingustic features change during the peer review and publishing process.
We find that preprints and biomedical text are quite similar, especially when controlling for the differential uptake of preprints across fields.
In other contexts, neural networks trained in certain ways produce a representation of words and documents that has useful properties for many tasks - termed word or document embeddings [@arxiv:1301.3781; @arxiv:1405.4053].
Here, we produce preprint embeddings, which we find distinguish different biomedical research areas and approaches.
We use these to produce a linguistic landscape of bioRxiv preprints, detect those that change substantially during publication, and identify journals that publish manuscripts that are linguistically similar to a target preprint.
We provide a web-app that projects a user-selected preprint onto this landscape and suggests journals and articles that are linguistically similar.
Taken together, our work reveals that preprints are, linguistically, similar to peer reviewed text and, while at a linguistic scale certain changes that occur during publication are modest, the extent of those changes is associated with different times to publication suggesting that comparisons between embeddings of preprints and peer reviewed documents provides a means to study the process of peer review.
