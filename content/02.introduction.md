...
For the rest of this paper, we explore the linguistic content contained within the bioRxiv repository and observe how this content changes when subjected to factors such as peer review.
In respect to textual content, we hypothesize that the content within bioRxiv would be almost identical to content within a biomedical repository of published articles such as PubMed Central [@doi:10.1073/pnas.98.2.381].
Noteworthy differences between these two repositories could be small-scale variations such as article category or subject differences.
Word embeddings have the ability to capture latent linguistic features [@arxiv:1301.3781; @arxiv:1405.4053] and we aimed to use this technique to excavate concepts hidden within the bioRxiv repository.
We hypothesize that concepts captured by these embeddings would tease out different areas of biomedical research.
In addition to excavating concepts captured, we extend our exploration to examine a number of practical uses for these embeddings.
We hypothesize that embeddings are able to detect whether a preprint has undergone drastic changes due to outside factors such as peer review.
Under this hypothesis, we examine how the magnitude of change is associated with a preprint's time to publication.
We also hypothesize that documents with similar linguistic content should have similar embedding representations.
If this hypothesis holds true, then we can utilize these embeddings to align preprints with their respective published versions or even identify journals that publish linguistically similar papers.
Lastly, we encapsulate our journal detection findings in the form of a web-app that users can use to find journals that publish linguistically similar papers to a preprint query.
