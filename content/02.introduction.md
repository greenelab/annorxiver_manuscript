...
To understand how they relate to the traditional publishing ecosystem, we examine the linguistic similarities and differences between preprints and peer reviewed text and obverse how linguistic features change during the peer review and publishing process.
We speculate that preprints and biomedical text would be quite similar, especially when controlling for the differential uptake of preprints across fields.
...
Here, we hypothesize that using these networks to embedded preprints is a versatile way to disentangle linguistic features and serve as a medium for improving preprint repository functionality.
We test this hypothesis by producing a linguistic landscape of bioRxiv preprints, detect those that change substantially during publication, and identify journals that publish manuscripts that are linguistically similar to a target preprint.
