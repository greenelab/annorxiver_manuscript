...
Documents within bioRxiv were slightly longer than those on PMCOA, but both were much longer than those from NYTAC (Table {@tbl:corpora_stats}).
...
The divergence of term frequency distributions between bioRxiv and PMCOA were low, especially among the top few hundred tokens (Figure {@fig:corpora_comparison_panels}A).
As more tokens were incorporated the Kullback–Leibler divergence started to increase, but remained much lower than when the biomedical corpora are compared against NYTAC.
These similarities support our notion that bioRxiv is a linguistically similar subset of the PMCOA repository.
