...
Across the entire list we found a high inter-rater reliability of with a Cohen's Kappa [@doi:10.1177/001316446002000104] of 0.92. <!-- CHECK THIS - IT WAS PRESENTED AS A PERCENT BUT I'M PRETTY SURE IT SHOULDN'T BE IF IT'S COHEN'S KAPPA -->
...
We treated examined this using the Kaplan-Meier estimator, considering the endpoint to be publication.
Of the most abundant preprint categories microbiology was the fastest to publish (point estimate, confidence intervals) and genomics was the slowest (point estimate, confidence intervals). <!--TODO: fill in the point estimates and confidence intervals-->
Though we did observe category-specific differences, these differences were generally modest.
One exception was the Scientific Communication and Education category, which took substantially longer to be peer reviewed and published (point estimate, confidence interval).
...
Users supply DOIs from _bioRxiv_ or _medRxiv_. <!-- we need to be consistent in how these are represented - should they always be italicized? it feels like a journal name would be - perhaps see what the bioRxiv folks did in their own paper. -->
...
The user-requested preprint's location in this space is then displayed.
Users can select regions to identify the terms most associated with those regions.
...
![
The journal recommender app workflow allows users to examine where an individual preprint falls in the overall document landscape.
A. Starting with the home screen, users can paste in a _bioRxiv_ or _medRxiv_ DOI, which sends a request to bioRxiv or medRxiv.
