...
](https://raw.githubusercontent.com/danich1/annorxiver/6bedc581a88b8d98043d847d234d06877458d898/pmc/journal_recommendation/output/figures/knn_result.png){@fig:knn_auc}

We sought to recommend journals based on the textual content of a paper.
We trained two different classifiers (centroid classifier and paper-paper classifier) to predict the journal endpoints for already published papers. 
We found that both clasifiers are better than our randomized baseline and that our centroid classifier acheived better accuracy on the test set compared to the paper-paper classifier (Figure {#fig:knn_auc}).
Despite the positive performance, recommending journals remains a challenging task as both model's overall accuracy is modest at best (Figure {#fig:knn_auc}).
...
](images/journal_recommender_workflow.svg){@fig:journal_rec_workflow}

We constructed an online app that provides users with journal suggestions based on their preprint content.
This tool takes in doi links from _bioRxiv_ and _medRxiv_ and provides the user with the following output: title of the query preprint, the top ten most similar papers, the top ten journal recommendations, where the preprint maps along the PMC landscape and individual bin information such as word odds ratios (Figure {#fig:journal_rec_workflow}).
