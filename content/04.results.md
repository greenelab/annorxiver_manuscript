...
### Measuring Time Delay for Preprint Publication Process

![
On average preprints suffer a 17-day delay before they are published.
...
The x-axis represents the Euclidean distance between document representations, while the y-axis represents the amount of days elapsed between a preprint posted on bioRxiv and the time a preprint is published.
...
](https://raw.githubusercontent.com/danich1/annorxiver/c5b782b91e388dd1143f0b3e42c59a1c8c3277e1/biorxiv/publication_delay_experiment/output/article_distance_vs_publication_time.png){#fig:distance_publication_time}

![
Preprints with multiple versions suffer an average of a 51-day delay to get published.
...
The x-axis represents the number of different versions a preprint has on bioRxiv, while the y-axis represents the amount of days elapsed between a preprint posted on bioRxiv and the time a preprint is published.
The color bar on the right represents the density of each square-bin in this plot where more dense regions have a brighter color compared to their counterparts.
](https://raw.githubusercontent.com/danich1/annorxiver/187e1ff87d679a928db0d5c88504971acfa11e9b/biorxiv/publication_delay_experiment/output/version_count_vs_publication_time.png){#fig:version_publication_time}

![
Biophysics, Biochemistry and Evolutionary biology preprint categories take less time to get published than their counterparts.
...
](https://raw.githubusercontent.com/danich1/annorxiver/c5b782b91e388dd1143f0b3e42c59a1c8c3277e1/biorxiv/time_to_publication/output/preprint_category_halflife.png){#fig:category_halflife}

The peer review process can result in varying amount of delays as preprints transition to being published.
We sought to quantify these time delays to provide insight on the review process demands.
We found that preprints take ~17 extra days to be published as authors meet peer-reviewed demanded changes (Figure {@#fig:distance_publication_time}).
We found that the average distance between preprints from the same category was 5.068 while the average distances for two random preprints within bioRxiv was 6.210. 
This suggests that a single distance unit represents approximately 18% change of a preprint's textual content.
We found that preprints are delayed on an average of ~51 days when multiple versions are required (Figure {@fig:version_publication_time}).
Lastly, we found that biophysics takes the least amount of time to have half of total number of posted preprints published, while zoology, clinical trials and pathology take the most time (Figure {@fig:category_halflife}).
