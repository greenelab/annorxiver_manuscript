...
We found that that average distance of two randomly selected papers from the bioinformatics category is 5.068, while the average distance of two randomly selected papers from bioRxiv is 6.210.
This suggests that pairs with distances closer to 5 units are as different as two papers within the same research field, while distances closer to 6.21 represent two distinctly different papers.
Overall, our findings support a notion that those preprints reviewed multiple times or those that are subject to larger revisions take longer to publish.
