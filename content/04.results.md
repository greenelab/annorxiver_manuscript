...
### Analyzing Preprints in Motion Collection

![
**A.** Published articles are focused on COVID-19 nomenclature and epidemiology while preprints are focused on experimental setup and instruments.
We plotted the 95% confidence interval for each reported token.
**B.** Of the tokens that differ between preprints and their published counterparts, the most abundant in preprints are "fviii",  "collagen", and "invasion" while the most abundant in published articles are  "$\pm$", "lipid", "ace2" and "cov-2".
**C.** Preprints are largely clustered within the center of the published landscape.
...
The red dots and red regression line represent preprints analyzed in [@doi:10.1101/2021.02.20.432090].
...
Preprints posted from January 1st, 2020 to April 30th, 2020 timeframe underwent moderately impacted during the peer review process and were published within a timely manner.
As preprints within this collection become published, the nomenclature for COVID-19 shifted from 2019-ncov to sars-cov-2 (Figure {@fig:covid_19_preprints}A).
Tokens enriched for preprints were focused on experimentation, while published articles contained tokens focused on reporting information about the COVID-19 pandemic (Figure {@fig:covid_19_preprints}A and {@fig:covid_19_preprints}B).
The $\pm$ token appears highly enriched for published articles (Figure {@fig:covid_19_preprints}) similar to our previous token analysis (Figure {@fig:corpora_comparison_panels}B).
Preprints in this collection are spread along the high-density region of the publication landscape, which signifies that this collection a diverse set of research topics (Figure {@fig:covid_19_preprints}C).
Regarding publication time, we found that preprints within this collection are published faster than the average preprint publication time  (Figure {@fig:covid_19_preprints}D and {@fig:covid_19_preprints}E}).
Each regression line resulted in a negative slope (Figure {@fig:covid_19_preprints}D and {@fig:covid_19_preprints}E}), signifying that COVID-19 preprints aren't significantly slowed down by the peer-review process.
