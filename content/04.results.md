...
The process of peer review includes a number of steps which take variable amounts of time [@doi:10.1002/nop2.51] and we first sought to measure if there is a difference in publication time between author-selected categories of preprints (Figure {@fig:publication_delay_panels}A).
...
Each distance unit in the embedding space corresponds to approximately sixteen additional days (Figure {@fig:publication_delay_panels}C).
A single distance unit in this context represents about  with distances greater than 5.068 representing pratically a full paper rewrite.
