...
![
The Preprints in Motion Collection results are similar those from all preprints, except that their time to publication was independent of the number of versions and amount of linguistic change.
**A.** Tokens that differed included those associated with typesetting and those associated with nomeclature of the virus that causes COVID-19.
...
**D.** The Preprints in Motion were published faster than other bioRxiv preprints and no dependence between the amount of lingustic change and time to publish was observed.
The x-axis shows the Euclidean distance between document representations of the first version of a preprint and its peer-reviewed form.
The y-axis shows the number of days elapsed between when the first version of a preprint is posted on bioRxiv and the time a preprint is published.
The color bar on the right represents the density of each hexbin in this plot where more dense regions are shown in a brighter color.
The red dots and red regression line represent Preprints in Motion.
...
The Preprints in Motion were not strongly colocalized in the lingustic landscape, suggesting that the collection covers a diverse set of research approaches (Figure {@fig:covid_19_preprints}C).
...
The relationship between time to publication and the number of versions (Figure {@fig:covid_19_preprints}D) and the relationship between time to publication and the amount of lingustic change (Figure {@fig:covid_19_preprints}E}) were both lost in the Preprints in Motion set.
Taken together, our findings suggest that that Preprints in Motion changed during publication in ways that were aligned with changes in the full preprint set but that peer review was accelerated in ways that broke the time dependences observed with the full bioRxiv set.
