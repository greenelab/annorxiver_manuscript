...
This tool takes in doi links from _bioRxiv_ and _medRxiv_ and provides the user with the following output: title of the query preprint, the top ten most similar papers, the top ten journal recommendations, the location where the query preprint falls along the PMC landscape and statistics on individual bins within the landscape (Figure {#fig:journal_rec_workflow}).
