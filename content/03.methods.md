...
We sought to predict the journal that a paper would eventually be published in based on its embeddings.
We examined predictions against both individual papers as well as journal centroids using k-nearest neighbor classifiers.
The first classifier recommended journals based on the articles in close proximity to the query article and the second classifier recommended journals based on the journal centroids in close proximity to the query article.
We used PubMed Central's (PMC) open access dataset to train both models.
A subset of bioRxiv preprints were directly linked to PMC papers because they had been published.
We held all of these out as our test set.
We used the remainder of the PMC corpus for training.
We embedded each PMC paper into the embeddings trained on bioRxiv preprints.
Furhermore, we filtered out journals that had fewer than 100 articles within PMC and downsampled the remaining journals to only have 100 articles, which prevented mega journals from completely dominating the predictions.
We compared two distance metrics (Eucledian distance and Manhattan distance) for both classifiers.
Our goal was to present the recommendations on a website as a short list.
We considered a list of roughly ten potential journals to be appropriate and considered a prediction to be correct if the correct journal appeared within the ten closest neighbors of a query.
For the paper-based classifier, because multiple papers may come from the same journal this may return fewer than ten potential matches.
