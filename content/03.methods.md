...
The optimum comparison for this set would likely be to consider only preprints posted on the same days as "Preprints in Motion" ones, but, based on our results examining publication rate over time, these may not yet have made it fully through the publication processes.
We performed a secondary analysis to control for the time since posting in which we filtered the bioRxiv snapshot to only contain publication pairs with publication time of less than or equal to 195 days.
