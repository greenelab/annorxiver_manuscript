...
We trained this model using the scikit-learn's [@scikit-learn] implementation of a randomized solver [@arxiv:0909.4061] with a random seed of 100, output of 50 principal components (PCs), and default settings for all other hyperparameters.
After training the model, every preprint within the bioRxiv corpus is asssigned a score for each generated PC.
We calculated the cosine similarity of each PC to all tokens within the CBOW model to visualize the concepts captured within each PC.
