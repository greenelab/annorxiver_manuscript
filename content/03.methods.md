...
The first classifier recommends journals based on articles in close proximity of the query, while the second classifier calculates the centroid of each journal and recommends journals based on close proximity.
We used PubMed Central's (PMC) open access dataset to train our model.
We reserved articles that were found in both PMC and bioRxiv for our test set and used the other articles for training.
We filtered out journals within our training set that have less than 100 articles.
Furthermore, we downsampled the remaining journals to only have 100 articles as this prevented overrepresentation of mega journals.
The first classifier was optimized using 10-fold cross validation on our downsampled training set.
Regarding our second classifier we calculated the centroids using the full filtered training set and then evaluated performance using the downsampled set.
We considered a positive to be if the correct journal appears within the top ten neighbors of the query and report the accuracy of each result.
