...
### Measuring Time Required for Preprint Publications
...
We calculated the difference between the date a preprint was posted onto bioRxiv and the date a preprint was accepted for publication. 
Along with calculating time differences, we calculated number of different versions each preprint may have and the difference between preprints and their corresponding published version's document embeddings. 
We performed linear regression to model the relationship between version count and a preprints time to publication as well as the relationship between document distances and a preprint's time to publication.
...
We generated survival curves using the Kaplan-Meier estimator [@doi:10.4103/0974-7788.76794] on preprints within the bioRxiv repository.
This estimator measures the duration of time it takes for a preprint to be published.
These curves were generated using the kaplanmeierfitter function from the lifelines (version 0.25.6) python package.
We used these curves to report half-life of each preprint category within bioRxiv.
We encountered some categories did not reach half of their preprints published, which results in an infinite amount of time duration.
In this case we replaced these infinite time measurements with the largest time difference across all preprints.
