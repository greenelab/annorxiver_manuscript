...
Because many tokens were unique to one set or the other and observed at low frequency, we focused on the union of the top 0.05% (~100) most frequently occuring tokens within each individual corpus.
...
The optimal number of tokens used to calculate the KL divergence is unknown, so we calculated this metric using a spectrum of 100 most frequently occuring tokens between two corpora to 5000 most frequently occuring tokens. 
