...
Linking preprints to their published versions is not a flawless process as preprints can drastically change as they undergo the revision process.
For example, preprints can change their titles right before publication (e.g. [@doi:10.1101/376665] and [@doi:10.1242/bio.038232]), which results in both the preprint and its published version being treated as distinct despite being the same article. 
We sought to remedy this issue by patching in missing preprint to published version links.
First, we used CrossRef [@doi:10.1629/uksg.233] to identify bioRxiv preprints  that already have links to their corresponding published version.
We filtered out links that contained papers not in PubMed Central's Open Access corpus.
Following the preprocessing step, we calculated the distribution of known preprint to published distances by taking the Euclidean distance between preprint embeddings and their corresponding published version embeddings.
We also calculated the null distribution which consisted of distances between preprints and a randomly selected article from the same venue as their corresponding published version.
Next, we calculated distances between preprints without a published version link with PubMed Central articles that aren't matched with a corresponding preprint.
We filtered out distances that were greater than the min value of our null distribution as these are considered false positives.
Lastly, we binned the remaining pairs based on percentiles of our true distribution and randomly sampled 200 articles from each bin for manual annotation.
