...
We sought to meausre the time it takes to get preprints through the peer review process.
We used biorxiv's api to obtain the following two dates: the date a preprint was posted onto biorxiv and the date a preprint was accepted for publication.
We calculated time offsets for every preprint by subtracting the preprint's published date from its corresponding posted date.
Following the time offset calculations, we obtained document distances by subtracting preprint's document representation with its final published version. 
We generated a scatterplot and regression line to observe the correlation between document distance offsets and time taken for publication.
We also generated a scatterplot and regression line to measure the time delay that results from preprints have multiple versions.
Authors sometimes post their preprints after they have published, which results in preprints receiving a negative time offset.
We kept preprints that have negative time in our plot analysis, but remmoved them when we calculated our surivival curves.
We generated survival curves using the kaplanmeierfitter function from the lifelines python package.
This function generates a Kaplan-Meier estimator [@doi:10.4103/0974-7788.76794] that estimates the survival time for un-published preprints within the bioRxiv repository.
We report the halflife of all preprint categories for our analysis.
Some categories didn't reach 50% and their reported time was replaced with the maximum time within our analysis.
