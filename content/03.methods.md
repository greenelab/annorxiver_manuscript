...
Through these prediction tasks, both networks learn latent linguistic features that can be used for downstream tasks, such as identifying similar tokens.
...
Preprints not already reported within bioRxiv's API were excluded from this analysis.
...
We encountered 123 cases where authors appeared to post preprints after the publication date, which resulted in preprints receiving a negative time difference, as previously reported [@url:https://medium.com/@OmnesRes/the-biorxiv-wall-of-shame-aa3d9cfc4cd7].
...
<div style="color:red">
Unpack this section a bit. 
Basically break down that journals some cofounders
	high publication rates
	publish articles from vastly different topic areas
Due to these two factors we developed a new way to handle that sort of issues.
</div>
...
The concept of identifying journals for papers is similar to [@doi:10.5195/jmla.2019.598]; however, our approach is substantially different from the author's work.
Our application downloads a pdf version of any preprint hosted on the bioRxiv or medRxiv server uses PyMuPDF [@url:https://pymupdf.readthedocs.io/en/latest/intro.html] to extract text from the downloaded pdf and feeds the extracted text into our CBOW model to construct a document embedding representation.
