...
### Linking Preprints to their Published Versions
Tracking preprints as they get published is a non-trivial task, which can result in preprints missing links towards their published version.
We sought to map preprints with their published version.
In particular we focused on matching preprints that currently do not have an established link.
First, we used CrossRef[@doi:10.1629/uksg.233] to map 31,385 biorxiv preprints with their corresponding published version.
Following the mapping process, we calculated the euclidean distance between preprint embeddings and their corresponding published embeddings.
We also calculated the euclidean distance of preprints and a random articles published from the same venue.
Next we calculated the distances of articles with no known links and the rest of published articles within pmc and kept pairings with distances there were at most the tail end of the preprint no known link distribution.
Then we binned each pair based on their distance cut off.
Lastly, we randomly sampled 200 articles from each bin and manually annotated each pair to determine if the pair is a match.
