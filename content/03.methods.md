...
We sought to measure the time required for preprints to go through the peer review process.
We queried bioRxiv’s api to obtain the date a preprint was posted onto bioRxiv as well as the date a preprint was accepted for publication.
We calculated the difference between a preprint's posted date and its corresponding publication date as well as counted the number of preprint versions posted onto bioRxiv.
We calculated the difference between preprints and their corresponding published version's document representation. 
...
We report both findings in the form of square bin plots.
Sometimes authors post their preprints after they have published, which results in preprints receiving a negative time difference.
We kept preprints that have negative time in our linear regression analysis but removed them in our survival curve analysis.

Document distances can be difficult to understand, so we sought to contextualize the meaning of a distance unit.
We grouped preprints into the following categories: preprints within the same journal, preprints within the same category and a conglomeration of all preprints within bioRxiv.
For each group we sampled two preprints and calculated their differences.
We repeated this process 1000 times and report the mean for each group.

Certain bioRxiv categories can take less time for their respective preprint to be published than other categories.
We modeled this trend by generating survival curves using the Kaplan-Meier estimator [@doi:10.4103/0974-7788.76794] on preprints within bioRxiv.
This estimator measures the duration of time passed for each preprint to be published.
These curves were generated using the KaplanMeierFitter function from the lifelines (version 0.25.6) python package.
We used these curves to report the half-life of each bioRxiv preprint category.
We encountered some categories that did not attain half of their preprints published.
This which results in an infinite time estimate and we replaced these instances with the maximum time difference across all preprints.
