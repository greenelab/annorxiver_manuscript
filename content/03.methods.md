...
In both cases, we optimized [WHAT NEEDED TO BE OPTIMIZED? THERE AREN'T A LOT OF FREE PARAMETERS HERE] the approaches by via 10-fold cross validation.
We evaluated performance of both classifiers on our gold standard test set of published preprints.

We developed a web application to make this prediction method available for all _bioRxiv_ preprints.
...
We pass this representation onto our journal venue classifiers to recommend journals based on the ten closest neighbors of individual papers as well as journal centroids.
[THIS USES KD TREES IN SKLEARN TO MAKE IT SCALABLE AND EFFICIENT AND I THINK THEY HAD TO BE SHARDED TO WORK GIVEN THE AVAILABLE RAM THOUGH THAT MIGHT NOT BE TRUE ANYMORE - GET THE DETAILS FROM DONGBO :) ]
...
We trained this model [ON WORD EMBEDDINGS DIRECTLY OR PCS AND ALSO WHAT DOCUMENTS?] using a hidden size of 2, a learning rate of 0.001, lambda_b of 0, lambda_c of 0.001, and lambda_d of 0.001 for 2000 iterations.
