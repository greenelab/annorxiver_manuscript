...
Since our approach is solely focused on embeddings, we utilized the k-nearest neighbors technique with euclidean distance to recommend journals.

First, we filtered all journals that had fewer than 100 papers in the PMC dataset.
A subset of our PMC corpus were directly linked to papers in bioRxiv as they had been published as open access articles.
We held out this subset and treated it as our gold standard test set.
The remainder of the PMC corpus was used for training and initial evaluation via cross validation.
We considered a list of roughly ten potential journals to be feasible for broadcasting to the user and we treated a prediction to be correct if the correct journal appeared within the ten closest neighbors of the query article.

Certain journals publish articles whose topics are remarkably niche, while others publish articles that cover a broad array of topics.
...
This classifier takes a query article that has been projected onto the embedding space trained on bioRxiv preprints as input and reports the journals of the top ten clostest papers.
The amount of journals returned via this method could be less than ten as multiple papers in close proximity to query article may belong to the same journal. 
...
Lastly, we evaluated performance of both classifiers on our golden standard test set.
