...
We excluded preprints that were not mentioned within bioRxiv's API from this analysis.
...
We encountered 123 cases where authors appeared to post preprints after the publication date, resulting in a negative time difference, as previously reported [@url:https://medium.com/@OmnesRes/the-biorxiv-wall-of-shame-aa3d9cfc4cd7].
...
Training classifiers to identify which journal publishes similar articles is challenging as not all journals are the same. 
...
Furthermore, some journals focus on publishing articles with a concentrated topic area, while others cover many dispersive topics.
Therefore, journals with a high publication rate and diverse set of article topics have a high chance of taking prevalence over a single classifier based on papers alone.
We designed two approaches to account for these characteristics - one centered on articles and another centered on journals.
...
We selected articles close to the query via Euclidean distance in the embedding space.
Once identified, we return the journal in which these articles were published.
We also return the articles that led to each journal being reported, as this approach allows for journals that frequently publish article to engulf our results.
...
This concept of identifying journals is similar to [@doi:10.5195/jmla.2019.598]; however, our approach of identifying journals is different from the author's work.
