...
### Contextualizing Preprints in Motion

During the initial stages of the COVID-19 pandemic, repositories such as bioRxiv and medRxiv received an influx of preprint submissions.
This influx along with prior skepticism on preprint efficacy [@doi:10.1101/2020.05.22.111294] prompted researchers to analyze preprint to publication changes in preprints posted within the January 1st 2020 to April 30th 2020 timeframe [@doi:10.1101/2021.02.20.432090]. 
We sought to contexualize this subset within our global picture of the bioRxiv preprint landscape.
We extracted all preprint digital object identifiers (doi) within [@doi:10.1101/2021.02.20.432090] and filtered out entries that corresponded to preprints within the medRxiv repository.
We manually downloaded the xml version of the remaining preprints and mapped their corresponding dois with their published counterparts using the same method mentioned in the Mapping bioRxiv Preprints to their Published Counterparts section. 
We used Pubmed Central's DOI converter [@url:https://www.ncbi.nlm.nih.gov/pmc/pmctopmid/] to map published article dois with their respective PubMed Central IDs.
We filtered out published articless that weren't apart of the PMCOA corpus and performed a token analysis that compared all preprints against their published versions (see Corpora Comparison).
Next, we generated document embeddings (see Constructing a Document Representation for Life Sciences Text) for every obtained preprint and published article.
We projected these preprint embeddings onto our publication landscape to visually observe the dispersion of this subset (see Web Application for Discovering Similar Preprints and Journals).
Finally, we performed a time analysis to observe how peer review changes affect a preprint's time to publication (see Measuring Time Duration for Preprint Publication Process).  
